...
import { generatePaymentInvoiceEmail, type PaymentInvoiceEmailData } from "@/lib/emailTemplates";
...
export async function POST(request: NextRequest) {
    ...
    let requestData: SendInvoiceRequest;
    try {
    ...
    try {
        // Fetch owner details including payment preference
        const ownerQuery = `
            SELECT
                id, owner_name, email, partner_email, payment_preference
        ...
        // Generate email
        const emailData: PaymentInvoiceEmailData = {
            ownerName: owner.owner_name.split(' ')[0], // First name only
            dogNames: dogNames,
            services: services,
            totalAmount: totalAmount,
            paymentPreference: owner.payment_preference || 'per_service'
        };

        const { subject, html } = generatePaymentInvoiceEmail(emailData);
        ...
        console.log(`Invoice sent to ${owner.owner_name} (${recipients.join(', ')}) - £${totalAmount.toFixed(2)}`);

        return NextResponse.json({
            success: true,
            message: `Invoice sent to ${owner.owner_name}`,
            details: {
